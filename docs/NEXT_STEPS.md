...
3. **Ambient lighting** — **hecho 2026-05-03**. Aline reacciona a `RenderSettings.ambient*` (color del entorno fluye al modelo entero — skin, BlackBody, Dress) sin necesidad de tocar materiales por evento. Implementación en tres capas:

   - **Include compartido `Assets/Aline/Shaders/AlineLighting.cginc`** con dos funciones: `AlineShade(N, base, floor, strength)` modulativo (`base * (floor + ambient*strength)`, para superficies con BaseColor visible — piel, ropa) y `AlineRimTint(N, base, fillStrength)` aditivo (`base + ambient*fill`, para superficies oscuras tipo BlackPart/Dress donde el rim debe levantarse con el entorno sin volverse gris).
   - **Refactor de 3 shaders custom** (`Aline/BodyLit`, `Aline/Fresnel`, `Aline/EnergyMask`) para llamar al helper. `Aline/Face` queda fuera intencionalmente (el void debe quedarse void); `Aline/Hair` también (cards puros).
   - **`SetRenderingSettings.ambientMode` + `ambientLight`/`ambientSkyColor`/etc** ya disparan cambios visibles en BS (Aline vira al color/brillo del ambient en tiempo real). Útil para FX narrativos: cambio de fase, fade outs, partículas con luz, recolor del skybox — todo propagado al personaje sin tocar nada del .prefab/.mat.

   **Gotcha gold (memoria `feedback_vivify_bundle_no_sh`):** Plan inicial era `ShadeSH9(unity_SHAr/g/b)` — el camino "correcto" Unity. **No funciona en Vivify bundles**: los SH coefficients requieren GI baking (Lighting → Generate Lighting) que no se ejecuta en standalone bundles. Pivot a `unity_AmbientSky/Equator/Ground` (poblados por Unity directamente desde `RenderSettings.ambient*` sin GI). El blend Trilight con fallback a Sky-flat-mode cubre los tres modos (Skybox, Trilight, Flat) sin oscurecer lados/abajo a negro.

   **Sandbox de validación**: dificultad **Normal** (`NormalStandard.dat`) cicla ambient cada 4 beats (Skybox → Flat blanco → Flat azul → Flat near-black → Skybox → Trilight) para validar que el shader engancha. Confirmado e2e en BS 2026-05-03. Mantener como herramienta de regression cuando tocamos lighting/shaders.
